  ...
  return (
    <div className="flex w-full max-w-md flex-col gap-1">
      <Boxed className={cn(bad && "border-destructive")}>
        {value.map((c) => (
          <div key={c} className="flex h-8 items-center gap-2.5 border-b border-border px-3">
        ...
        <p className={cn("tw-label", value.length === 0 ? "text-warning" : "text-muted-foreground")}>
          ...
          {!sameSet(value, defaults) && (
            <button
              type="button"
              disabled={disabled}
              className="ml-2 text-foreground underline-offset-2 hover:underline disabled:pointer-events-none"
