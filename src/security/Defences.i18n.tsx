...
export const guardText = messages(
  {
    ...
    inspectTools: {
      ...
      where: "证据在「流量」页：命中的请求带有标记，停在上面可看到命中了哪条规则。",
    },
    scanConfigs: {
      title: "配置面扫描",
    ...
    observeTip:
      "「观察」照常检测并记录，但不改变任何请求。可根据记录结果决定是否切换到「拦截」。",
    oldCore: "core 版本较旧，未提供防护状态。升级后可使用此页面。",
    nowOff: "当前：不检测，不记录。",
    nowObserve: (em: Em) => (
      <>
        当前：检测并记录，{em("不改变任何请求")}。检测结果显示在「安全 ›
        发现」中。
      </>
    ),
    ...
    rulesAppended: (em: Em) => (
      <>
        自定义规则{em("追加")}
        在内置规则之后，不替换内置规则，新版本增加的内置规则同样生效。
      </>
    ),
  ...
  },
  {
    ...
    redact: {
      ...
      where: "The evidence is on the Traffic page: a request it touched carries a redaction mark.",
    },
    inspectTools: {
      ...
      where:
        "The evidence is on the Traffic page: a matching request is marked, and hovering it names the rule.",
    },
    scanConfigs: {
      title: "Config scan",
      ...
      where: "The evidence is under Findings, beside this tab.",
    },

    noVersion: "The config version has not been read yet. Try again shortly.",
    intro:
      "Each of the three defenses has three modes; all default to Observe.",
    ...
    oldCore:
      "This core version is too old to report protection status. The page is available after upgrading.",
    nowOff: "Currently: nothing is detected or recorded.",
    nowObserve: (em: Em) => (
      <>
        Currently: detects and records {em("without changing any request")}.
        Results appear under Security › Findings.
      </>
    ),
    ...
    rulesAppended: (em: Em) => (
      <>
        Custom rules are {em("appended")} to the built-in rules and do not
        replace them; built-in rules added in new versions also apply.
      </>
    ),
    rulesCount: (added: number, disabled: number) =>
    ...
    scopeNote: (em: Em) => (
      <>
        Outbound redaction decides whether anything is redacted; this table
        lists {em("the categories redacted for each upstream")}. Official
        endpoints are not redacted by default.
      </>
    ),
    howToChange: "How to change",
    howToChangeTip:
      "Edit the upstream in Upstreams and set redaction in its Security section.",
    upstream: "Upstream",
    trust: "Trust",
