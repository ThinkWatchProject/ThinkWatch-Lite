...
export const securityPageText = messages(
  {
    ...
    /**
     * 页头上日志那段时间里命中了几次。`n` 是画好的数字；读到的是前若干条时
     * 带「+」。自定义区间的名字本身是「9/8 至今」，不再加「内」
     */
    hits: (n: ReactNode, range: string, custom: boolean) => (
      <>
  ...
  },
  {
    ...
    hits: (n: ReactNode, range: string, custom: boolean) => (
      <>
        {n} {custom ? `matches ${lower(range)}` : `matches in the last ${range}`}
      </>
    ),
